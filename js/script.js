...
function updateFabCart() {
    ...
    if (cart.length === 0) {
        itemsContainer.innerHTML = '<div class="empty-fab-cart" style="padding: 20px; text-align: center; color: #999;">Your cart is empty</div>';
    ...
    cart.forEach(function(item) {
        ...
        html += '<div class="fab-cart-item"><div class="fab-cart-item-image"><img src="images/' + item.image + '" alt="' + item.name + '" onerror="this.src=\'https://placehold.co/50x50/e31e24/white?text=' + encodeURIComponent(item.name.charAt(0)) + '\'"></div><div class="fab-cart-item-details" style="flex:1;"><div class="fab-cart-item-name">' + item.name + '</div><div class="fab-cart-item-price">' + item.quantity + ' x UGX ' + item.price.toLocaleString() + '</div></div></div>';
    });
...
function loadCartItems() {
    ...
    var cart = getCart();
    if (cart.length === 0) {
        container.innerHTML = '<div class="empty-cart"><i class="fas fa-shopping-cart"></i><p>Your cart is empty</p><a href="index.html" class="shop-now-btn">Continue Shopping</a></div>';
        updateCartSummary();
        return;
    }
    container.innerHTML = '';
    cart.forEach(function(item) {
        ...
        cartItem.innerHTML = '<div class="cart-item-image"><img src="images/' + item.image + '" alt="' + item.name + '" onerror="this.src=\'https://placehold.co/100x100/e31e24/white?text=' + encodeURIComponent(item.name.charAt(0)) + '\'"></div><div class="cart-item-details"><h3 class="cart-item-name">' + item.name + '</h3><div class="cart-item-price">UGX ' + item.price.toLocaleString() + '</div><div class="cart-item-actions"><div class="quantity-control"><button class="decrease">-</button><input type="number" value="' + item.quantity + '" min="1"><button class="increase">+</button></div><div class="remove-item"><i class="fas fa-trash"></i> Remove</div></div></div>';
    ...
    header.innerHTML = '<h2>Your Items (' + cart.length + ' items)</h2><button class="clear-cart" id="clearCartBtn"><i class="fas fa-trash-alt"></i> Clear Cart</button>';
...
function setupProductGallery() {
    ...
    if (relatedGrid) {
        ...
        related.forEach(function(rel) {
            ...
            var card = document.createElement('div');
            card.className = 'product-card';
            card.innerHTML = '<div class="product-image"><img src="images/' + rel.images[0] + '" alt="' + rel.name + '" onerror="this.src=\'https://placehold.co/300x200/e31e24/white?text=' + encodeURIComponent(rel.name) + '\'">' + (discount > 0 ? '<span class="discount-badge">' + discount + '% OFF</span>' : '') + '</div><div class="product-info"><h3 class="product-name">' + rel.name + '</h3><div class="product-price"><span class="current-price">UGX ' + rel.price.toLocaleString() + '</span></div><button class="add-cart-btn">View Product</button></div>';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var fabCheckout = document.querySelector('#fabCheckoutWhatsapp');
    if (fabCheckout) fabCheckout.addEventListener('click', proceedToCheckout);
});
